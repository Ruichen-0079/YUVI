...
>
> Current implemented baseline is A0–A6 plus A7.1, A7.2, the A8.1 journal contract, A8.2a PostgreSQL Journal storage and A8.2b conversational receipt admission. See [`implementation-baseline.md`](implementation-baseline.md). Current Future authority is indexed in [`README.md`](README.md).
...
A0–A6, A7.1, A7.2, A8.1, A8.2a and A8.2b are implemented. A8.2c–A8.2f and A9–A12 remain planned engineering. Aggregate A8.2 remains incomplete until all six A8.2 leaves close. A8.1, A8.2a and A8.2b validation are recorded in [`v0.1.3-a8.1-journal-contract.md`](../validation/v0.1.3-a8.1-journal-contract.md), [`v0.1.3-a8.2a-journal-store.md`](../validation/v0.1.3-a8.2a-journal-store.md) and [`v0.1.3-a8.2b-conversational-ingress.md`](../validation/v0.1.3-a8.2b-conversational-ingress.md).
...
A8.2a implements the database ownership rule: Journal durability consumes the Supervisor/deployment-owned PostgreSQL infrastructure and remains available when Mem0 is the active Memory backend. `MEMORY_REPOSITORY=postgres` / long-term Memory backend selection does not determine whether the Journal exists, and no second PostgreSQL daemon is introduced. A8.2b now gates ordinary conversational HTTP/SSE/WebSocket input through durable RECEIPT append; speech observation handoff, standalone vision and local controls remain planned in later leaves.
...
- ordinary conversational HTTP/SSE/WebSocket inputs become durable receipts before Runtime semantic processing; speech, standalone-vision and governed local-control inputs remain later A8.2 leaves and each surface preserves its identity/audience/retention limits;
