...
Status: **IMPLEMENTED REALITY**. A8.1 and A8.2a–A8.2e3 source and verification are recorded in [their validation records](../validation/v0.1.3-a8.1-journal-contract.md), [A8.2a](../validation/v0.1.3-a8.2a-journal-store.md), [A8.2b](../validation/v0.1.3-a8.2b-conversational-ingress.md), [A8.2c](../validation/v0.1.3-a8.2c-speech-ingress.md), [A8.2d](../validation/v0.1.3-a8.2d-vision-ingress.md), [A8.2e1](../validation/v0.1.3-a8.2e1-product-controls.md), [A8.2e2](../validation/v0.1.3-a8.2e2-runtime-controls.md) and [A8.2e3](../validation/v0.1.3-a8.2e3-proactive-consent.md). This is the only Future document that states what exists today. Every other Future document states decisions, planned engineering or hypotheses and must defer to this record and to source. When source changes, the atom that changes it updates this file.
...
A0–A6, A7.1–A7.2, A8.1, A8.2a–A8.2d and A8.2e1–A8.2e3 are complete and are not redesigned by the new architecture. A0–A6 original atom specifications are preserved in git history at `ecbb3c5` (the previous text of [the v0.1.3 roadmap](09-v0.1.3-platform-completion.md)); their validation records remain the evidence:
...
| A8.2e1 | Host-built CONTROL receipts precede Product configuration/Person persistence and proactive resume; Product queue held through append; identity stays unresolved and private fields are withheld | [A8.2e1](../validation/v0.1.3-a8.2e1-product-controls.md) |
| A8.2e2 | Runtime-preflighted P8 correction and read-text control commands receive minimal CONTROL receipts before P8 append or process-local grant mutation; no source dedup, identity resolution, OUTCOME or durable grant replay | [A8.2e2](../validation/v0.1.3-a8.2e2-runtime-controls.md) |
| A8.2e3 | Desktop/Tauri settings remain authored durable consent; local READY projections are journaled before volatile Runtime apply; UNKNOWN_DENIED fails closed immediately without a receipt or persisted authored false | [A8.2e3](../validation/v0.1.3-a8.2e3-proactive-consent.md) |
...
| Product control receipt admission | Host-only `HostProductControlReceiptAdmission` in `apps/server/src/product-control-receipt-admission.ts` builds bounded CONTROL receipts; Product routes preserve their local guard and queue, then call the existing Product store/Runtime owners |
| Runtime-governed control receipt admission | Host-only `HostRuntimeControlReceiptAdmission` in `apps/server/src/runtime-control-receipt-admission.ts` admits only P8 correction and read-text authorization summaries; Runtime retains P8 validation/append and process-local read-text grant ownership |
| Proactive consent authority and projection | Desktop/Tauri settings are durable authored policy; `HostProactiveConsentReceiptAdmission` journals accepted READY projections, while Runtime owns the volatile revision fence/effective eligibility; UNKNOWN_DENIED is immediate and receipt-free |
...
3. **Partial production durable causal ingress.** A8.2b commits ordinary `/message`, `/v1/messages`, `/v1/messages/stream` and `/ws` user-message receipts before Runtime. A8.2c commits one finalized-speech receipt before transcription observations become handoff-ready or `/v1/voice/message` enters semantic Runtime processing. Speech principal and Person binding remain unresolved; caller speaker/profile hints and acoustic matches do not authenticate. The Runtime capture reservation and pending handoff are volatile, so receipts survive restart but turns are not replayed and no restart-durable dedup is claimed. Preview STT remains outside finalized admission. A8.2d commits a minimal standalone vision input receipt before provider invocation; image bytes and URLs remain outside Journal, and provider results are not recorded. A8.2e1 commits minimal CONTROL receipts before Product configuration/Person store operations and proactive resume; local controller identity is unresolved and private settings/profile fields are omitted. A8.2e2 commits minimal CONTROL receipts before P8 correction append and process-local read-text grant creation; correction/path content is withheld and read-text grants are not restored after restart. A8.2e3 keeps Desktop/Tauri settings as authored durable consent, journals accepted READY projections before applying volatile Runtime consent, and uses immediate receipt-free UNKNOWN_DENIED invalidation for settings failure/change. Runtime policy persistence does not become a second authored consent store; its legacy consentEnabled field is ignored in production projection mode. No Person binding is inferred, no source dedup is claimed, and projection state is not replayed from Journal after restart. A8.2f1 commits minimal CONTROL receipts before direct voice-profile enrollment, profile-to-saved-Person binding, profile deletion and explicit binding removal. Voice receipts keep caller principal/Person unresolved, retain no label/audio/embedding, and leave enrollment audio NOT_RETAINED; existing Memory-before-reference-index and binding/profile/sample deletion sequences remain unchanged. A8.2f2 Product review and multi-recording enrollment remain planned, as does aggregate applicability closure for proactive-turn, standalone TTS, production manual Memory mutation and Live2D presentation administration. Chat image bytes and raw speech audio remain outside Journal; the separate bounded voice-review buffer remains unchanged. Existing conversation, Runtime, A3 and Memory records remain distinct, and durable effect accounting remains A9.
