...
Status: **PRODUCTION ARCHITECTURE DECISION**. A8.1's versioned command/envelope contract, A8.2a's PostgreSQL append store, A8.2b's ordinary conversational HTTP/SSE/WebSocket receipt gate, A8.2c's finalized-speech receipt gate, A8.2d's standalone-vision receipt gate and A8.2e1–A8.2e3's Product/Runtime/proactive-consent receipt gates are **IMPLEMENTED REALITY**. A8.2f voice closure, A9 effect accounting and A10 lineage remain planned. Existing conversation, Memory and delivery records are partial foundations, not substitutes for the Journal. See the [A8.1 contract](../validation/v0.1.3-a8.1-journal-contract.md), [A8.2a store](../validation/v0.1.3-a8.2a-journal-store.md), [A8.2b ingress](../validation/v0.1.3-a8.2b-conversational-ingress.md), [A8.2c speech ingress](../validation/v0.1.3-a8.2c-speech-ingress.md), [A8.2d vision ingress](../validation/v0.1.3-a8.2d-vision-ingress.md), [A8.2e1 Product controls](../validation/v0.1.3-a8.2e1-product-controls.md), [A8.2e2 Runtime controls](../validation/v0.1.3-a8.2e2-runtime-controls.md) and [A8.2e3 proactive consent](../validation/v0.1.3-a8.2e3-proactive-consent.md) records.
...
## Product / Person control receipt boundary

A8.2e1's host-only `HostProductControlReceiptAdmission` commits `RECEIPT/CONTROL` for exactly `PUT /product/configuration`, `POST /product/people` and `POST /product/proactive/resume`. The local-dashboard guard runs first. Configuration requests are strictly shape-checked, revision-checked, normalized into a cloned candidate, and provider-registry-validated before admission. The route's existing mutation queue stays held through append and the existing save/apply path, so a second same-revision save waits and then conflicts without its own receipt. No live or durable Product settings mutate before receipt commit.

The retained TEXT payload is a host-generated JSON summary, not a copy of the request. Configuration stores only the operation and expected revision; it excludes API keys, provider IDs, URLs, model details and configuration fragments. Person create allocates the target ID before append; update uses the ID of a verified existing Product Person. Summaries include only operation, target Person ID and requested-primary flag; display names, notes and persona values remain outside Journal. Person target IDs are not placed in authority subjects and do not resolve the controller principal. Principal/binding remain `UNRESOLVED`, subjects are empty, audience is `UNKNOWN`, occurrence time is `UNKNOWN`, and no source-dedup identity is supplied. Local IP and development dashboard token are not identity evidence.

Receipt means only that the locally authorized control request was accepted for processing. Product save/reload, packaged speech configuration, downstream `persistProfileEvidence`, and proactive wake retain their existing owners and response semantics. A post-commit Runtime reload or profile-evidence failure leaves the receipt; no OUTCOME/DERIVATION, retry, rollback or A10 lineage is added. The former broad A8.2e leaf was source-split into e1 Product/Person routes, e2 Runtime-governed local controls and e3 proactive-consent authority closure. A8.2e3 separately covers the local settings projection route; it does not turn the e1 proactive-resume route into an authored consent store.

## Proactive-consent projection receipt boundary

A8.2e3 leaves Desktop/Tauri settings as the authored durable source of `proactive.enabled`. The local-only `/v1/proactive/consent` endpoint projects a current SettingsView into Runtime; it does not save settings or authenticate the author. A READY projection that would change or establish effective consent is preflighted, committed as a bounded `RECEIPT/CONTROL` summary, and then revalidated before Runtime apply. It records only `{operation: "proactive.consent.settings-projection", enabled, settingsRevision}` with unresolved principal and Person binding, no subjects and unknown audience. The revision orders projections but is not source identity or source dedup. Exact replay is a no-op.
