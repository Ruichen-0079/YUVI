...
Status: **PRODUCTION ARCHITECTURE DECISION**. A8.1's versioned command/envelope contract, A8.2a's PostgreSQL append store, A8.2b's ordinary conversational HTTP/SSE/WebSocket receipt gate and A8.2c's finalized-speech receipt gate are **IMPLEMENTED REALITY**. Standalone vision and local-control ingress remain planned in A8.2d–f. A9 effect accounting and A10 lineage remain planned. Existing conversation, Memory and delivery records are partial foundations, not substitutes for the Journal. See the [A8.1 contract](../validation/v0.1.3-a8.1-journal-contract.md), [A8.2a store](../validation/v0.1.3-a8.2a-journal-store.md), [A8.2b ingress](../validation/v0.1.3-a8.2b-conversational-ingress.md) and [A8.2c speech ingress](../validation/v0.1.3-a8.2c-speech-ingress.md) records.
