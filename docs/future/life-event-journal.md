...
Receipt means only that the locally authorized control request was accepted for processing. Product save/reload, packaged speech configuration, downstream `persistProfileEvidence`, and proactive wake retain their existing owners and response semantics. A post-commit Runtime reload or profile-evidence failure leaves the receipt; no OUTCOME/DERIVATION, retry, rollback or A10 lineage is added. The former broad A8.2e leaf was source-split into e1 Product/Person routes, e2 Runtime-governed local controls and e3 proactive-consent authority closure. In particular, `POST /v1/proactive/consent` remains open in e3 and is not covered by the e1 proactive-resume route.
