...
Status: **PRODUCTION ARCHITECTURE DECISION**. A8.1's versioned command/envelope contract and A8.2a's PostgreSQL append store are **IMPLEMENTED REALITY**; production ingress remains **PLANNED ENGINEERING**, A8.2b–f. A9 effect accounting and A10 lineage remain planned. Existing conversation, Memory and delivery records are partial foundations, not substitutes for the Journal. See the [A8.1 contract record](../validation/v0.1.3-a8.1-journal-contract.md) and [A8.2a store record](../validation/v0.1.3-a8.2a-journal-store.md).
