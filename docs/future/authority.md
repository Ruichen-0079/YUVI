...
Status: **PRODUCTION ARCHITECTURE DECISION**. The A8.1 protocol contract, A8.2a PostgreSQL append store, A8.2b conversational receipt gate, A8.2c finalized-speech receipt gate and A8.2d standalone-vision receipt gate are **IMPLEMENTED REALITY**; governed local-control ingress remains **PLANNED ENGINEERING** in A8.2e–f. Existing owners are recorded in [implementation baseline](implementation-baseline.md). One meaning has one active committing owner in a deployment epoch. A storage writer is not automatically an authority over the meaning it stores.
...
| Normalized conversational receipt | A8.2b host facade `apps/server/src/conversational-receipt-admission.ts` normalizes ordinary text/route facts; Runtime still owns semantics | Governed local-control receipt adapters remain A8.2e–f; ordinary message routes infer no authenticated principal, Person binding or audience membership |
