...
Status: **PRODUCTION ARCHITECTURE DECISION**. The A8.1 protocol contract, A8.2a PostgreSQL append store and A8.2b conversational receipt gate are **IMPLEMENTED REALITY**; voice, vision and governed local-control ingress remain **PLANNED ENGINEERING** in A8.2c–f. Existing owners are recorded in [implementation baseline](implementation-baseline.md). One meaning has one active committing owner in a deployment epoch. A storage writer is not automatically an authority over the meaning it stores.
...
| Normalized conversational receipt | A8.2b host facade `apps/server/src/conversational-receipt-admission.ts` normalizes ordinary text/route facts; Runtime still owns semantics | Voice, vision and local-control receipt adapters remain A8.2c–f; no authenticated principal, Person binding or audience membership is inferred from these routes |
...
A model can propose an action, label an authorized evidence window, suggest a correction or express an interpretation. It cannot commit a scalar relationship state, choose its own evidence eligibility, authenticate a principal, grant a plugin permission or rewrite policy. A validated model output remains a measurement with an error model, not an authoritative fact. A8.1 validates producer commands against a separately supplied authority snapshot; A8.2a's append authority assigns event identity, namespace, recorded time and serialized commit sequence. A8.2b supplies known conversational route facts through a host-only facade and admits ordinary input only after the Journal commit succeeds. It still records unresolved principal/Person binding and unknown audience because those routes have no authentication or membership authority.

A database transaction enforces a durable commit; it does not turn a claim into truth. Conversely, passing through Runtime does not by itself make an unsafe command valid. The remaining A8.2c–f admission seams must check the applicable source facts, scope, audience, parent evidence, current execution, effect contract, policy version and any real stable source identity before those inputs are accepted.
